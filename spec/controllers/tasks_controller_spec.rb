...
RSpec.describe TasksController, type: :controller do
  ...
  let(:valid_attributes) {
    {description: 'test description', user_id: User.first.id}
  }
  ...
  describe "PUT #update" do
    context "with valid params" do
      let(:new_attributes) {
        {description: 'updated description', is_complete: true}
      }
      ...
      it "updates the requested task" do
        ...
        task.reload
        expect(task.description).to eql('updated description')
      ...
      it "does not update the task if it does not belong to the current user" do
        task = Task.create! valid_attributes.merge(user_id: User.create(name: 'user2', password: 'password').id)
          put :update, params: {id: task.to_param, task: new_attributes}, session: valid_session
          task.reload
          expect(task.description).to eql('test description')
          expect(task.is_complete).to eql(nil)
      end
    end
  ...
  describe "DELETE #destroy" do
    ...
    it "does not destroy the task if it does not belong to the current user" do
      task = Task.create! valid_attributes.merge(user_id: User.create(name: 'user2', password: 'password').id)
      expect {
        delete :destroy, params: {id: task.to_param}, session: valid_session
